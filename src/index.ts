...
import { Builder, Linter, LiteralBlock, IfBlock, RepeatBlock, CommentBlock } from 'rockplate';
...
const getSchemaResolver = (document: { uri: string }) => {
  return utils.getSchemaResolver(document, sendRequest);
};
...
const markupCodeBlock = (language: string, code: string) => {
  return '```' + language + '\n' + code + '\n```';
};
